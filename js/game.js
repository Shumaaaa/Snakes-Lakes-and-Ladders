...
window.addEventListener('load', () => {
  ...
  if (!players[0].isHuman) setTimeout(_doRoll, 1000);
});
...
function _doRoll() {
  ...
  setTimeout(() => { if (pendingRoll !== null) _executeMove(pendingRoll); }, 8000);
}
...
function _executeMove(roll) {
  ...
  animateGlide(currentTurn, prevPos, targetPos, players, () => {
    ...
    drawFullBoard(players);
    _refreshUI();
    setTimeout(() => _handleSpecial(result.special, roll, targetPos), 8000);
  });
}
...
function _afterSpecial(roll) {
  ...
  if (roll === 6 && p.state === 'normal' && !gameOver) {
    ...
    if (!p.isHuman) setTimeout(_doRoll, 400);
    return;
  }
  busy = false;
  _advanceTurn();
...
function _advanceTurn() {
  ...
  pendingRoll = null;
  _refreshUI();
  if (!players[currentTurn].isHuman) setTimeout(_doRoll, 400);
}
